...
<div class="admin-page">
  ...
  <form method="POST" action="<?= BASE_PATH ?>/admin/job/edit/<?= $job->id ?>" class="admin-form">
      <?= csrf_field() ?>
    <div class="form-row">
      <div class="form-group col-md-8">
        <label for="title">Tiêu đề <span class="required">*</span></label>
        <input type="text" id="title" name="title" class="form-control" value="<?= htmlspecialchars($job->title) ?>"
          required>
      </div>
    ...
    <div class="form-row">
      <div class="form-group col-md-6">
        ...
        <select id="location" name="location" class="form-control">
          ...
          <option value="Hà Nội" <?= $job->location === 'Hà Nội' ? 'selected' : '' ?>>
            Hà Nội
          </option>
          <option value="TP.HCM" <?= $job->location === 'TP.HCM' ? 'selected' : '' ?>>
            TP.HCM
          </option>
          <option value="Đà Nẵng" <?= $job->location === 'Đà Nẵng' ? 'selected' : '' ?>>
            Đà Nẵng
          </option>
        </select>
      </div>
      ...
      <div class="form-group col-md-6">
        ...
        <select id="category_id" name="category_id" class="form-control">
          ...
          <?php 
                    // Render option danh mục việc làm theo cây và đánh dấu danh mục đang chọn khi sửa tin.
                    function renderCategoryOptionsEdit($categories, $selectedId, $level = 0) {
                        foreach ($categories as $cat) {
                            ...
                            if (!empty($cat->children)) {
                                renderCategoryOptionsEdit($cat->children, $selectedId, $level + 1);
                            }
                        }
                    }
                    renderCategoryOptionsEdit($categoryTree ?? [], $job->category_id);
                    ?>
        </select>
      </div>
    </div>
    ...
    <div class="form-row">
      <div class="form-group col-md-4">
        ...
        <input type="number" id="salaryMin" name="salaryMin" class="form-control" value="<?= $job->salary_min ?>"
          min="0">
      </div>

      <div class="form-group col-md-4">
        <label for="salaryMax">Lương tối đa</label>
        <input type="number" id="salaryMax" name="salaryMax" class="form-control" value="<?= $job->salary_max ?>"
          min="0">
      </div>
      ...
      <div class="form-group col-md-4">
        ...
        <select id="type" name="type" class="form-control">
          ...
          <option value="Contract" <?= $job->type === 'Contract' ? 'selected' : '' ?>>
            Hợp đồng
          </option>
    ...
    <div class="form-row">
      ...
      <div class="form-group col-md-6">
        ...
        <select id="featured" name="featured" class="form-control">
          <option value="0" <?= $job->featured === '0' ? 'selected' : '' ?>>Không</option>
          <option value="1" <?= $job->featured === '1' ? 'selected' : '' ?>>Có</option>
        </select>
      </div>
    </div>
    ...
    <div class="form-row">
      <div class="form-group col-md-6">
        ...
        <input type="text" id="thumbnail" name="thumbnail" class="form-control"
          value="<?= htmlspecialchars($job->thumbnail ?? '') ?>">
      </div>
